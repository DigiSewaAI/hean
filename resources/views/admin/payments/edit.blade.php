...
<div style="max-width:800px; margin:0 auto; background:#fff; border-radius:12px; border:1px solid #e2e8f0; padding:24px;">
    ...
    <form action="{{ route('admin.payments.update', $payment) }}" method="POST">
        ...
        <div style="margin-bottom:16px;">
            ...
            <input type="number" step="0.01" name="amount" id="amount" value="{{ old('amount', $payment->amount) }}" placeholder="{{ __('messages.enter_amount') }}" style="width:100%; padding:8px 12px; border:1px solid #e2e8f0; border-radius:6px; @error('amount') border-color:#ef4444; @enderror">
        ...
        <div style="margin-bottom:16px;">
            ...
            <input type="text" name="transaction_id" id="transaction_id" value="{{ old('transaction_id', $payment->transaction_id) }}" placeholder="{{ __('messages.enter_transaction_id') }}" style="width:100%; padding:8px 12px; border:1px solid #e2e8f0; border-radius:6px;">
        ...
        <div style="margin-bottom:16px;">
            ...
            <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date', $payment->payment_date->format('Y-m-d')) }}" style="width:100%; padding:8px 12px; border:1px solid #e2e8f0; border-radius:6px; @error('payment_date') border-color:#ef4444; @enderror">
        ...
        <div style="margin-bottom:16px;">
            ...
            <input type="text" name="bank_name" id="bank_name" value="{{ old('bank_name', $payment->bank_name) }}" placeholder="{{ __('messages.enter_bank_name') }}" style="width:100%; padding:8px 12px; border:1px solid #e2e8f0; border-radius:6px;">
        ...
        <div style="margin-bottom:16px;">
            ...
            <input type="text" name="bank_account" id="bank_account" value="{{ old('bank_account', $payment->bank_account) }}" placeholder="{{ __('messages.enter_bank_account') }}" style="width:100%; padding:8px 12px; border:1px solid #e2e8f0; border-radius:6px;">
        ...
        <div style="margin-bottom:24px;">
            ...
            <textarea name="remarks" id="remarks" rows="3" placeholder="{{ __('messages.enter_remarks') }}" style="width:100%; padding:8px 12px; border:1px solid #e2e8f0; border-radius:6px;">{{ old('remarks', $payment->remarks) }}</textarea>
            @error('remarks') <span style="color:#ef4444; font-size:0.8rem;">{{ $message }}</span> @enderror
        </div>

        <div style="display:flex; gap:10px;">
            <button type="submit" style="background:#F59E0B; color:#fff; padding:10px 24px; border:none; border-radius:6px; cursor:pointer; font-weight:600;">
